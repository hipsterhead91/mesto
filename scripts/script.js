...
// РЕНДЕР КАРТОЧЕК НА ЭКРАНЕ
function renderCard(item) {
  const card = getCard(item);
  cardsContainer.prepend(card);
  document.querySelector('.element__like-button').addEventListener('click', like);
  document.querySelector('.element__delete-button').addEventListener('click', deleteCard);
